...
# READ IN DATA ------------------------------------------------------------
# file <- c("ECFC_P09")
id_sample <- c("ECFC_P09","ECFC_P08","ECFC_P19","ECFC_P26")
# load the LUT
...
list_datasc <- lapply(id_sample,function(x){
  data <- Read10X(data.dir = paste0("../../../raw_data/cellranger/",x,"/outs/filtered_feature_bc_matrix/"))
  datasc <- CreateSeuratObject(counts = data, project = LUT %>%
                                 filter(sample_id == x) %>%
                                 pull(passage), min.cells = 20, min.features = 200)
  
  # datasc@meta.data
  datasc$percent.mt <- PercentageFeatureSet(datasc, pattern = "^MT-")
  # datasc$stage <- LUT %>%
  #   filter(sample_id == "ECFC_P09") %>%
  #   pull(stage)
  ...
  datasc$test <- datasc@meta.data %>%
    ...
    mutate(test = percent.mt > 1 & percent.mt < 10 & nFeature_RNA > 2000 & nFeature_RNA < 9000) %>%
    pull(test)
...
# save the plot
ggsave("out/image/fixed_scatter_feature_mito.pdf",width = 5,height = 4)

meta_total %>%
  ggplot(aes(y = percent.mt,x = nFeature_RNA,col=not_outlier)) + geom_point(alpha=0.3)+ facet_wrap(~orig.ident) + theme_bw()+
  theme(strip.background = element_blank(), 
        panel.border = element_rect(colour = "black", fill = NA))
# save the plot
ggsave("out/image/adaptive_scatter_feature_mito.pdf",width = 5,height = 4)
...
# save the plot
ggsave("out/image/fixed_boxplot_reads.pdf",width = 8,height = 4)

#
meta_total %>%
  gather(var,value,c(nCount_RNA,nFeature_RNA,percent.mt)) %>%
  filter(var == "percent.mt") %>%
  ggplot(aes(x=value))+geom_histogram(binwidth = 0.05) + facet_wrap(orig.ident~var,scales = "free")+theme_bw()+scale_x_log10() + geom_vline(xintercept = c(1,10),col="red")+
  theme(strip.background = element_blank(), 
        panel.border = element_rect(colour = "black", fill = NA))
# save the plot
ggsave("out/image/fixed_histo_mito.pdf",width = 5,height = 4)

meta_total %>%
  gather(var,value,c(nCount_RNA,nFeature_RNA,percent.mt)) %>%
  filter(var == "nFeature_RNA") %>%
  ggplot(aes(x=value))+geom_histogram(binwidth = 0.05) + facet_wrap(orig.ident~var,scales = "free")+theme_bw()+scale_x_log10() + geom_vline(xintercept = c(2000,9000),col="red")+
  theme(strip.background = element_blank(), 
        panel.border = element_rect(colour = "black", fill = NA))
# save the plot
ggsave("out/image/fixed_histo_features.pdf",width = 5,height = 4)
...
meta_total %>%
  ...
  theme(strip.background = element_blank(), 
        panel.border = element_rect(colour = "black", fill = NA))
# save the plot
ggsave("out/image/fixed_scatter_mito.pdf",width = 10,height = 10)
...
colnames(list_datasc_fixed_norm_doublet$ECFC_P08@meta.data)
c("orig.ident","nCount_RNA","nFeature_RNA","percent.mt","mt_bin","test","not_outlier","RNA_snn_res.0.8","seurat_clusters","pANN","DF")

lapply(list_datasc_fixed_norm_doublet, function(x){
  meta <- x@meta.data
  colnames(meta) <- c("orig.ident","nCount_RNA","nFeature_RNA","percent.mt","mt_bin","test","not_outlier","RNA_snn_res.0.8","seurat_clusters","pANN","DF")
  x@meta.data <- meta
  head(x@meta.data)
  dim(x@meta.data)[1]
})
...
pmap(list(names(list_datasc_fixed_norm_doublet),list_datasc_fixed_norm_doublet),function(x,y){
  ...
  # head(y@meta.data)
  #
  saveRDS(object = y,file = paste0("../out_large/Belfast_scRNAseq_ECFC_senescence/object/datasc_fix_filter_norm_doublet_",x,".rds"))
})
...
pmap(list(names(list_datasc_fixed_norm_doublet),list_datasc_fixed_norm_doublet),function(x,y){
  ...
  saveRDS(object = datasc_filter,file = paste0("../out_large/Belfast_scRNAseq_ECFC_senescence/object/datasc_fix_filter_norm_doublet_Singlet_",x,".rds"))
})
